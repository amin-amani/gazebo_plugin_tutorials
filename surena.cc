...
namespace gazebo
{
  class Surena : public ModelPlugin
  {
...
private: common::PID pid2;
  ...
  {
    std::cerr << "Invalid joint count, Surena plugin not loaded\n";
    return;
  }
...
//   this->model->GetJointController()->SetVelocityTarget(this->joint2->GetScopedName(), 0);
   ...
   // Just output a message for now
      std::cerr << "\nThe surena plugin is attach to model[" <<_model->GetName() << "]\n";      
      // Create the node
...
ros::SubscribeOptions so =
  ros::SubscribeOptions::create<std_msgs::Float32>(
      ...
      1,
      boost::bind(&Surena::OnRosMsg, this, _1),
      ros::VoidPtr(), &this->rosQueue);
this->rosSub = this->rosNode->subscribe(so);

// Spin up the queue helper thread.
this->rosQueueThread =
  std::thread(std::bind(&Surena::QueueThread, this));
    }
...
// Tell Gazebo about this plugin, so that Gazebo can call Load on this plugin.
  GZ_REGISTER_MODEL_PLUGIN(Surena)
}
#endif
